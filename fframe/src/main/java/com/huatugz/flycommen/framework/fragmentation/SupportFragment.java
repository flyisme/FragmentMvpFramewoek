...
/**
 * Created by flyisme on 2017/8/21.
 */
...
public class SupportFragment extends RxFragment implements ISupportFragment {
    ...
    /**
     * 修复软键盘遮挡输入框与沉浸式顶栏的冲突 view为顶部view且父控件为RelativeLayout
     * 跟布局结合fitsSystemWindows="true"一起使用
     */
    protected void fixInputSoft(View view, final EditText... et) {
        fixInputSoft(view);
    ...
    /**
     * 确保根布局是RelativeLayout
     * @param view 第一个子View
     */
    protected void fixInputSoft(View view)
    {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    private boolean hasFocus(EditText[] et) {
        if (et == null || et.length == 0) {
            return false;
        }
        boolean flag = false;
